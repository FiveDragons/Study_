...
'''
# 파일 생성하기
f = open("새파일.txt", 'w') # f라는 파일을 오픈하고 '새파일.txt'이라는 파일을 상대주소로 만들고 'w'(Write) 쓰다.
f.close    # 파일을 닫겠다.
'''
...
# 절대주소 : 처음부분(C:/)부터 주소를 써주는 것  (ex => "C:/Python/새파일.txt")
# 상대주소 : 현재 실행하는 파일을 기준으로 상대적인 경로를 써주는 것 (ex => "새파일.txt")

'''
# 쓰기모드 : 파일을 쓰기 모드로 열어 출력값 적기
f = open("새파일.txt", 'w', encoding="UTF-8") # encoding을 통해 한글이 깨지지 않게 해준다.
...
f = open("새파일.txt", 'r', encoding="UTF-8") # read를 통해 값을 읽어온다.
while True:                 # 무한반복 
    line = f.readline()     # 새파일.txt에 있는 내용을 불러올때 사용함
...
lines = f.readlines()     # lines는 새파일의 내용을 한줄씩 담고있는 list이다.
for line in lines:
...
'''
# 추가모드
f = open("새파일.txt", 'a', encoding="UTF-8") # add를 통해 값을 추가한다.
...
with open("새파일2.txt", "w", encoding="UTF-8") as f: # with을 활용하여 새파일2.txt에 새로 작성하고 as를 사용해서 변수(f)에 저장한다.
